...
async def on_startup(dispatcher):
    # await bot.send_message(,'Запуск бота. Восстановление данных... ')
    # try:
    ...
    await bot.send_message(, 'База данных полностью восстановлена, бот готов к работе. База данных будет обновляться каждые 4 часа')
...
async def start_posts_interval():
    ...
    while not stop_event.is_set():
        if posts_count < max_posts_per_day:
            ...
            if posts_count >= max_posts_per_day:
                await bot.send_message(, f"Достигнут дневной лимит отправки сообщений. ({max_posts_per_day})")
...
async def send_message_to_channel(image, message, endtime, coupon_id):
    ...
    if endtime is not None:
        ...
        # await bot.send_message(, f'Пост будет удален {delete_time}')
        # await bot.send_message(, f'Пост будет удален {delete_time}')
